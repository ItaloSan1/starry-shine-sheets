...
Manual-backed reading (corrected after tracing sheet ES0366J, see `cases/Z13513-1861/diagnosis-walkthrough.md`): the six chassis sensors share **one 5 V supply P109ANG (GR/WH) that the DCON itself generates at J32-20** ("STEER SNSR PWR (5.0 VDC)") and **one sensor ground P110RT (BK) at J32-19**; this supply does **not** cross the rotator. The TCON's own 5 V at J12-26/25 feeds the boom angle sensors. The fault table for "Value at 0 V" says check 5 V at the sensor, wiring, and the **5.0 VDC LED on the TCON board**. Six chassis sensors at 0 V while the TCON can still display them means the DCON is alive on CAN (a dead DCON shows "DCON CAN Bus", propel disabled) and its 5 V output, its J32 pin/wire, or a short on the chassis sensor bus is the fault. SCON off the bus drops all six safety relays. Staged plan (pins verified against SM pp.208–210; full detail in the case file):
...
- **Stage 2 5 V references**: TCON 5.0 VDC LED lit and 5 V at the boom angle sensor connectors J114/J154 (TCON supply). Then DCON J32-20 to J32-19: 5 V. If the DCON side reads 0 V, unplug J107–J110, J160, J161 one at a time; 5 V returns → that sensor or its branch of chassis harness 94956GT is shorted; still 0 V with all unplugged → harness pin 20 shorted, or DCON 101004GT 5 V output dead.
